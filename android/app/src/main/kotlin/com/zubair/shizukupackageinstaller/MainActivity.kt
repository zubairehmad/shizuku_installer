...
class MainActivity : FlutterActivity() {
	...
	@Volatile
	private var activeStagedPaths: MutableList<String>? = null
	@Volatile
	private var activeExtractedDir: File? = null
		...
		Thread {
			...
			try {
				...
				val packageName = payload.packageName
					?: resolvePackageNameFromApk(payload.apkFiles)
				...
				payload.apkFiles.forEachIndexed { index, apkFile ->
					throwIfCancelled()
					Log.d(TAG, "Staging split APK ${index + 1}/${payload.apkFiles.size}: ${apkFile.name}")
					val staged = stageApkToShellTmp(
				...
				if (isManifest) {
					zipFile.getInputStream(entry).use { input ->
						...
						val packageFromManifest =
							manifest.optString("package_name").ifBlank {
	...
	private fun cleanupExtractedXapk(tempDir: File) {
		try {
			if (tempDir.exists()) {
				...
				if (!deleted) {
					Log.w(TAG, "Failed to delete extracted XAPK via app IO; trying Shizuku rm: ${tempDir.absolutePath}")
	...
	private fun runInstallMultipleCommand(stagedPaths: List<String>): CommandResult {
		...
		return lastResult
			?: CommandResult(-1, "", "Unable to execute install-multiple command.")
	}
	...
	private fun installUsingSession(stagedPaths: List<String>): CommandResult {
		...
		val createResult = runShizukuCommand(
			"pm install-create -r --user 0",
			30,
			registerProcess = true,
		)
		if (createResult.exitCode != 0) {
			Log.w(TAG, "install-create failed: ${createResult.errorOutput.ifBlank { createResult.output }}")
			return createResult
		}
		...
		stagedFiles.forEachIndexed { index, file ->
			...
			val writeCommand =
				"pm install-write -S $size $sessionId \"$name\" \"${file.absolutePath}\""
	...
	private fun stageApkToShellTmp(apkPath: String, token: String): String {
		...
		val process = createShizukuProcess(arrayOf("sh", "-c", "cat > \"$targetPath\""), null, null)
		...
		if (chmodResult.exitCode != 0) {
			Log.w(TAG, "Failed chmod on staged APK: ${chmodResult.errorOutput.ifBlank { chmodResult.output }}")
		}
		return targetPath
	}
